...
def extract_text_from_image(file_path: Path) -> str:
    """Extract text from image using OpenCV preprocessing + OCR."""
...
def extract_text(file_path: Path) -> str:
    """Choose correct extraction method based on file type."""
...
def normalize_question(raw_text: str) -> str:
    """Clean spaces and remove numbering."""
...
def looks_like_question(text: str) -> bool:
    """Check whether line looks like a question."""
    ...
    first_word = (
        first_word_match.group(0).lower()
        if first_word_match
        else ""
    )
    ...
    return (
        "?" in candidate
        or first_word in QUESTION_STARTERS
    )
...
def extract_questions_from_text(
    ...
    for candidate in candidates:
        ...
        if (
            question_key not in seen
            and looks_like_question(question)
        ):
            questions.append(
                ExtractedQuestion(
                    question=question,
...
def preprocess_question(
    ...
    no_punctuation = lowercase_text.translate(
...
def build_similarity_report(
    ...
    """Build repeated and rare question reports."""

    sklearn_text = import_or_raise(
        "sklearn.feature_extraction.text",
        "scikit-learn"
    )

    sklearn_metrics = import_or_raise(
    ...
    max_similarity_by_index = [
        0.0 for _ in raw_questions
    ]

    similar_to_by_index = [
        "" for _ in raw_questions
    ]
...
def process_files(
    ...
    for file_path in input_files:
        ...
        try:
            ...
            print(
                f"{file_path.name}: extracted {len(questions)} questions"
            )
            ...
            print(
                f"[ERROR] Could not process {file_path}: {exc}"
            )
    ...
    repeated_path = (
        output_folder / "repeated_questions.csv"
    )

    rare_path = (
        output_folder / "rare_questions.csv"
    )

    similarity_path = (
        output_folder / "similarity_report.csv"
    )
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description=(
            "Separate repeated and rare questions "
...
def main():
    ...
    summary = process_files(
        [Path(file_name) for file_name in args.files],
        args.threshold,
        Path(args.output),
    )
    ...
    print(
        f"Number of questions extracted: "
        f"{summary['total_questions']}"
    )

    print(
        f"Repeated question count: "
        f"{summary['repeated_count']}"
    )

    print(
        f"Rare question count: "
        f"{summary['rare_count']}"
    )
